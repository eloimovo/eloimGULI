                ...
                <aside class="c-attr-wrap">
                    <section class="ml20 mr15">
                        ...
                        <section class="c-attr-mt" v-else>
                            <a href="#" title="立即购买" class="comm-btn c-btn-3" @click=" createOrder"
                               >立 即 购 买</a
                                >
                        </section>
                    </section>
...
export default {
  methods: {
      //生成订单
    createOrder() {
    orderApi.createOrder(this.course.courseId).then(response =>{
        //获取返回的订单号
        //生成订单之后，跳转订单显示页面
    ...
    //获取课程详细信息
    getCourseInfo() {
      courseApi.getCourseFrontInfo(this.course.courseId).then((resp) => {
    ...
    initComment() {
      comment
        . getCommentPage(this.page, this.limit, this.course.courseId)
        .then((response) => {
    ...
    gotoPage(page) {
      comment. getCommentPage(page, this.limit, this.courseId).then((response) => {
        this.data = response.data.data;
      });
    },
  },
